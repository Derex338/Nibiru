...
public sealed partial class FancyTechnologyInfoPanel : Control
{
    ...
    public FancyTechnologyInfoPanel(TechnologyPrototype proto, bool hasAccess, ResearchAvailability availability, SpriteSystem sprite)
    {
        ...
        var ConstSys = _ent.System<ConstructionSystem>();
        ...
        InitializeRecipeUnlocks(proto, lathe, sprite, ConstSys);
    ...
    private void InitializeRecipeUnlocks(TechnologyPrototype proto, LatheSystem lathe, SpriteSystem sprite, ConstructionSystem ConstSys)
    {
        UnlocksContainer.RemoveAllChildren();
        foreach (var recipeId in proto.RecipeUnlocks)
        {
            var recipe = _proto.Index(recipeId);
            UnlocksContainer.AddChild(new MiniRecipeCardControl(proto, _proto, sprite, lathe, ConstSys, recipe));
        }
		foreach (var craftId in proto.CraftUnlocks)
        {
            var craft = _proto.Index(craftId);
            UnlocksContainer.AddChild(new MiniRecipeCardControl(proto, _proto, sprite, lathe, ConstSys, null, craft));
        }
    }
