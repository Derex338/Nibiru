## Животноводство

nibiru-livestock-verb-examine = Осмотреть животное
nibiru-livestock-resource-ready = [color=green]✓ {$resource}[/color] — готов к сбору ({$yield} шт.)
nibiru-livestock-resource-growing = [color=yellow]⟳ {$resource}[/color] — рост: {$percent}%
nibiru-livestock-pregnant = [color=cyan]Беременна[/color] — {$percent}%
nibiru-livestock-ready-to-breed-female = [color=green]Готова к спариванию[/color]
nibiru-livestock-ready-to-breed-male = [color=green]Готов к спариванию[/color]
nibiru-livestock-sex = Пол: {$sex ->
    [Male] Самец
    [Female] Самка
    *[other] Неизвестно
}

## Приручение

nibiru-animal-tamed = [color=green]Прирученное[/color] — доверие: {$trust}%
nibiru-animal-wild = [color=red]Дикое[/color]

## Настроение

nibiru-animal-mood = Настроение: {$mood ->
    [Happy] [color=green]Счастливое [/color]
    [Content] [color=lightgreen]Довольное[/color]
    [Sad] [color=yellow]Грустное [/color]
    [Angry] [color=orange]Злое [/color]
    [Wild] [color=red]Одичавшее [/color]
    *[other] Неизвестно
}

## Страх маунта

nibiru-mount-fear = Страх: {$fear}% | Тренировка: {$training}%

## Команды животных

nibiru-animal-command-follow = Следуй за мной
nibiru-animal-command-stay = Стоять
nibiru-animal-command-attack = Атаковать
nibiru-animal-command-guard = Охранять
nibiru-animal-command-search = Искать
nibiru-animal-command-deliver = Доставить

## Привязь

nibiru-leash-attached = Вы привязали {$animal} верёвкой.
nibiru-leash-detached = Вы отвязали {$animal}.
nibiru-leash-broke-free = {$animal} вырвалось из привязи!

## Стрижка и дойка

nibiru-harvest-shearing = Вы стрижёте {$animal}...
nibiru-harvest-milking = Вы доите {$animal}...
nibiru-harvest-complete = Собрано: {$resource} x{$amount}

## Территория

nibiru-territorial-warning = {$animal} предупреждающе рычит — вы на его территории!

## Сон

nibiru-animal-sleeping = {$animal} крепко спит.
nibiru-animal-waking = {$animal} просыпается.

## Огонь

nibiru-animal-fears-fire = {$animal} в ужасе от огня!

## Стая

nibiru-pack-leader-died = Вожак стаи убит! Стая в панике!
nibiru-pack-formation = Стая сформирована: {$count} членов.

## Окно Тренировки

nibiru-animal-training-title = Обучение и Дрессировка
nibiru-animal-training-trust = Уровень Доверия
nibiru-animal-training-stress = Устойчивость к Стрессу (Маунт)
nibiru-animal-training-train-stress = Тренировать Устойчивость
nibiru-animal-training-commands = Команды
nibiru-animal-training-learned = Изучено
nibiru-animal-training-teach = Обучить
nibiru-animal-training-verb = Окно Дрессировки
nibiru-animal-training-trust-low = Животное не доверяет вам достаточно или сильно устало! Покормите его.
nibiru-animal-training-success = Животное успешно выучило команду: {$command}!
nibiru-animal-training-stress-success = Животное стало более устойчиво к стрессу!
nibiru-animal-training-req-trust = Требуется {$trust} доверия
nibiru-animal-command-follow-all = Вы подзываете всех своих животных.
nibiru-animal-command-stay-all = Вы приказываете всем своим животным стоять.
nibiru-animal-command-attack-target = Вы натравливаете своих животных на цель!
nibiru-animal-command-follow-start = {$animal} начинает следовать за вами.
nibiru-animal-command-follow-single = Вы приказываете животному следовать.
nibiru-animal-command-stay-single = Вы приказываете животному стоять на месте.
nibiru-animal-command-attack-single = Вы отправляете животное атаковать!
nibiru-animal-command-grab-single = Вы приказываете животному схватить цель!
nibiru-animal-command-attack-mode = Режим атаки активирован. Укажите на цель.
nibiru-animal-command-grab-mode = Режим захвата активирован. Укажите на цель.
nibiru-animal-command-mode-cancel = Режим команды отменён.
nibiru-animal-command-search-success = Вы отправляете животное искать!
nibiru-animal-command-search-fail = На этом предмете нет следов!
nibiru-animal-command-deliver-start = Вы отправляете животное на доставку.
nibiru-animal-command-too-far-to-hear = Животное слишком далеко, чтобы услышать вас!
nibiru-animal-commander-too-far = Животное слишком далеко и больше не слушается.

## Захват животным

nibiru-animal-grab-detaching = Вы пытаетесь оторвать {$animal} от себя...
nibiru-animal-grab-detached = Вы оторвали {$animal} от себя!


nibiru-pigeon-post-rename-verb = Переименовать голубятню
